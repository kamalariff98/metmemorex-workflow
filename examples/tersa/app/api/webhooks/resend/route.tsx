  ...
  try {
    ...
    if (!env.SUPABASE_AUTH_HOOK_SECRET) {
      return NextResponse.json(
        { error: { http_code: 500, message: 'Supabase auth hook secret not configured' } },
        { status: 500 }
      );
    }
    ...
    if (!env.RESEND_TOKEN) {
      return NextResponse.json(
        { error: { http_code: 500, message: 'Resend token not configured' } },
        { status: 500 }
      );
    }
    ...
    if (!env.RESEND_EMAIL) {
      return NextResponse.json(
        { error: { http_code: 500, message: 'Resend email not configured' } },
        { status: 500 }
      );
    }

    // Create Resend client only after checking configuration
