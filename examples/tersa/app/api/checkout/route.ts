...
export const GET = async (request: NextRequest) => {
  ...
  if (!stripe) {
    return new Response('Stripe is not configured for this environment', { status: 503 });
  }

  if (!env.STRIPE_HOBBY_PRODUCT_ID || !env.STRIPE_PRO_PRODUCT_ID || !env.STRIPE_USAGE_PRODUCT_ID) {
    return new Response('Stripe is not configured for this environment', { status: 503 });
  }
