...
export const env = createEnv({
  ...
  server: {
    ...
    STRIPE_HOBBY_PRODUCT_ID: z.string().min(1).startsWith('prod_').optional(),
    STRIPE_PRO_PRODUCT_ID: z.string().min(1).startsWith('prod_').optional(),
    STRIPE_USAGE_PRODUCT_ID: z.string().min(1).startsWith('prod_').optional(),
    STRIPE_CREDITS_METER_ID: z.string().min(1).startsWith('mtr_').optional(),
    STRIPE_CREDITS_METER_NAME: z.string().min(1).optional(),
    STRIPE_WEBHOOK_SECRET: z.string().min(1).startsWith('whsec_').optional(),

    SUPABASE_AUTH_HOOK_SECRET: z.string().min(1).startsWith('v1,whsec_').optional(),

    // Supabase Integration - required
    POSTGRES_URL: z.string().url().min(1),
    SUPABASE_SERVICE_ROLE_KEY: z.string().min(1),

    // AI SDK - make most optional for development
    OPENAI_API_KEY: z.string().min(1).startsWith('sk-').optional(),
    GOOGLE_GENERATIVE_AI_API_KEY: z.string().min(1).optional(),
    GROQ_API_KEY: z.string().min(1).startsWith('gsk_').optional(),
    DEEPSEEK_API_KEY: z.string().min(1).startsWith('sk-').optional(),
    ...
    TOGETHER_AI_API_KEY: z.string().min(1).startsWith('tgp_v1_').optional(),
    COHERE_API_KEY: z.string().min(1).optional(),
    VERCEL_API_KEY: z.string().min(1).startsWith('v1:').optional(),
    MISTRAL_API_KEY: z.string().min(1).optional(),
    HUME_API_KEY: z.string().min(1).optional(),
    LMNT_API_KEY: z.string().min(1).optional(),
    PERPLEXITY_API_KEY: z.string().min(1).startsWith('pplx-').optional(),
    ...
    BF_API_KEY: z.string().min(1).optional(),
  },
  client: {
