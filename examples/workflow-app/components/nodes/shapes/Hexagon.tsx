...
type HexagonData = { label?: string; onLabelChange?: (value: string) => void; color?: string | null };
...
export const HexagonNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div className={`px-2 ${selected ? 'ring-4 ring-gray-500 rounded' : ''}`} onDoubleClick={() => setIsEditing(true)}>
      <svg width="100" height="100" viewBox="0 0 100 100" className="w-full h-full">
        ...
        <path d={hexagonPath} fill={customColor || `url(#hex-gradient-${id})`} stroke={selected ? '#3b82f6' : '#1e40af'} strokeWidth={selected ? 3 : 2} />
        <foreignObject x="10" y="35" width="80" height="30">
          <div className="flex items-center justify-center h-full">
            {isEditing ? (
              <form onSubmit={(e) => (e.preventDefault(), commit(text))} className="w-full">
                <input ref={inputRef} type="text" value={text} onChange={(e) => setText(e.target.value)} onBlur={() => commit(text)} className="w-full bg-transparent border-none outline-none text-center text-inherit font-inherit text-sm" />
              </form>
            ) : (
