...
const CircleNode = ({ data, selected }) => {
  ...
  const getNodeStyles = () => {
    ...
    const sizeClasses = data.size === 'small' ? 'w-16 h-16 text-xs' :
                       data.size === 'large' ? 'w-28 h-28 text-lg' :
                       'w-20 h-20 text-sm'; // medium
  ...
  const nodeStyles = customColor 
    ? `rounded-full border-2 transition-all duration-200 cursor-pointer select-none flex items-center justify-center ${data.size === 'small' ? 'w-16 h-16 text-xs' : data.size === 'large' ? 'w-28 h-28 text-lg' : 'w-20 h-20 text-sm'} ${
  ...
  return (
    ...
    >
      ...
      {isEditing ? (
        ...
        <div className="text-center break-words px-2">{data.label}</div>
      )}
