...
const RectangleNode = ({ data, selected }) => {
  ...
  const getNodeStyles = () => {
    ...
    const sizeClasses = data.size === 'small' ? 'text-sm min-w-[80px] min-h-[40px]' :
                       data.size === 'large' ? 'text-lg min-w-[140px] min-h-[60px]' :
                       'text-base min-w-[100px] min-h-[50px]'; // medium
  ...
  const nodeStyles = customColor 
    ? `px-4 py-3 border-2 transition-all duration-200 cursor-pointer select-none ${data.size === 'small' ? 'text-sm min-w-[80px] min-h-[40px]' : data.size === 'large' ? 'text-lg min-w-[140px] min-h-[60px]' : 'text-base min-w-[100px] min-h-[50px]'} ${
